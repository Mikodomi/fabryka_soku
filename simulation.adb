...
procedure Simulation is
   ...
   task body Buffer is
      ...
      function Needed_Amount_For_Assemblies
        (Product : Producer_Type) return Integer
      is
         Count : Integer := 0;
      begin
         for Z in Assembly_Type loop
            Count := Count + Assembly_Content (Z, Product);
         end loop;
         return Count;
      end Needed_Amount_For_Assemblies;

      function Can_Accept (Product : Producer_Type) return Boolean is
      begin
         if In_Storage >= Storage_Capacity then
            return False;
         elsif Storage(Product) >= Needed_Amount_For_Assemblies (Product) then
             return False;
         else
            return True;
         end if;
